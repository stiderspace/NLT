 ...
 * GraphicalRepresentation or a GraphicalRepresentation using standard settings: <br>
 * Basic shape:<br>
 * <code><br>
...
public class GraphicalRepresentation extends JLabel {
	/**
	 * Determines if labels are to be added to any new graphical representations
	 * by default
	 */
	public static boolean addLabels = false;

	/*
	 * Constant value indicating the amount of space to save for the
	 * representation's label.
	 */
	...
	/*
	 * The source image for this graphical representation is kept in
	...
	/*
	 * The horizontal offset from the top left corner of the representation to
	 * it's center, measured in pixels. This value is used to calculate the
	 * correct location of the representation in the GUI.
	 */
	private double offsetX;

	/*
	 * The vertical offset from the top left corner of the representation to
	 * it's center, measured in pixels. This value is used to calculate the
	 * correct location of the representation in the GUI.
	 */
	...
	/*
	 * The object's rotation (orientation) in the previous frame is stored in
	 * previousRotation. This allows update to check if a new image has to be
	 * generated when the image is rotating
	 */
	...
	/*
	 * The object's diameter in pixels within the image. Usually the image
	 * dimensions are bigger than the part of the image representing the object.
	 * To be able to scale the image correctly this value must be set.
	 */
	 ...
	 *            Any PhysicalObject which needs to be visible
	 */
	public GraphicalRepresentation(PhysicalObject physicalObject) {
		...
		// SIZE is rounded up to make sure the representation fits in the
		// container
		int size = (int) Math.ceil(Simulator.pixelsPerMeter
				* physicalObject.diameter);
		offsetX = (size + 1) / 2;
		offsetY = (size + 1) / 2;
	 ...
	 * @param diameterInImage
	 *            diameter of the image
	 */
	public GraphicalRepresentation(PhysicalObject physicalObject,
			String imagePath, int diameterInImage) {
		this(physicalObject);
		...
		if (imagePath == null) {
			...
			try {
				...
				Debug.printError("An error occurred while reading file "
						+ imagePath + "\n" + e.getMessage());
				sourceImage = null;
				return;
			}
		...
		scale = (Simulator.pixelsPerMeter * physicalObject.diameter)
				/ diameterInImage;
	...
	public GraphicalRepresentation(PhysicalObject physicalObject, String name, int diameter, Color aColor) {
		this(physicalObject, name, diameter);
		this.color = aColor;
	}
	 ...
	 *            diameter of the image
	 * @param rotating
	 *            should the image rotate?
	 */
	public GraphicalRepresentation(PhysicalObject physicalObject,
			String imagePath, int diameterInImage, boolean rotating) {
		this(physicalObject, imagePath, diameterInImage);
		this.rotating = rotating;
	}

	/**
	 * Gets the horizontal offset from the top left corner of the representation
	...
	/**
	 * Updates the representation's location, and if neccesary the image. If the
	 * representation not an image it draws the alternative representation.
	 * 
	 * @param g
	 *            TODO PARAM: DOCUMENT ME!
	 */
...
//			g.fillOval(m - 1, m - 1, 3, 3);
		...
		} else { // An image exists, so prospone super method to end
	 ...
	 * world dimensions, the simulator resolution and the offset values for this
	...
	private Point getGUILocation() {
		...
		int y = (int) ((Simulator.pixelsPerMeter * (World.HEIGHT - worldY))
				- offsetY + .5);
		return new Point(x, y);
	}

	/**
	 * Generates the rotated and scaled version of the image acting as graphical
	 * representation.
	 * 
	 * @return $returnType$ TODO RETURN: return description
	 */
	private BufferedImage getTransformedImage() {
		...
		AffineTransform rotationMatrix = AffineTransform
				.getRotateInstance(-physicalObject.rotation, sourceWidth / 2.0,
						sourceHeight / 2.0);
		transformationMatrix.concatenate(rotationMatrix);

		AffineTransformOp transformation = new AffineTransformOp(
				transformationMatrix, AffineTransformOp.TYPE_BILINEAR);

		BufferedImage image = transformation.filter(sourceImage, null);
		int newWidth = Math.min(image.getWidth(),
				(int) ((sourceWidth * scale) + 1));
		int newHeight = Math.min(image.getHeight(),
				(int) ((sourceHeight * scale) + 1));
	 ...
	 * 
	 * @param layer
	 *            What layer to use, these are statics in the Simulator class.
	 */
