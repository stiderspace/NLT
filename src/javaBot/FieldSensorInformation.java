 ...
 * 
 * @version $Revision$ last changed 03-07-2006
 * 
...
public abstract class FieldSensorInformation extends NonMovableObject {
	...
	public double[] giveSensoryInformationTo(Robot r) {
		...
		try {
			...
			// Loop through the robot's sensor to locate a FieldSensor (and
			// MouseSensor)
			for (int i = 0; i < sar.length; i++) {
				if (sar[i] instanceof FieldSensor) {
					...
					Line sensorLine = fs.sensorLine(r.getLocation().getX(), r
							.getLocation().getY(), r.rotation);

					// Location robotLocation = r.getLocation();
					// Location relativeSensorLocation = s.getPosition();
					// Location sensorLocation = new
					// Location(robotLocation.getX()
					// + relativeSensorLocation.getX(), robotLocation.getY()
					// + relativeSensorLocation.getY());

					Point location = Simulator
							.toPixelCoordinates(sensorLocation);
					// int pg = pixelGrab.handlepixels(getImage(), location.x,
					// location.y, 1, 1);
					int pg = pixelGrab.handlePixels(getImage(), location.x,
							location.y, 8, 8);
			...
			Debug.printError("An error occurred in " + name + ": "
					+ e.toString());
		}
	 ...
	 * 
	 * @param width
	 *            : <strong>absolute<strong> width of Image
	 * @param heigth
	 *            : <strong>absolute<strong> width of Image
	 */
