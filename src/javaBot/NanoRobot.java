 ...
 * simulation using the openVBB simulator with wiring as per the JPB controller
 * with electical connections provided as per the JPB specification.
 * 
 * The UVMRobot can also act as an avatar for a real robot. In particular the
 * UVMRobot implements the same AgentWebService interface that is available on
 * the remote Agent robot. This means it looks like the remote robot to
 * applications accessing it.
 * 
 * @author James Caska The UVMRobot proxy implements the
 */
public abstract class NanoRobot extends UVMRobot implements StepTimeListener,
		ADCSampleListener, DigitalOutputPinListener, AgentWebService {
	/**
	...
	UVMICD icd; // The ICD is the gateway to the real-device for programming and
	 ...
	 */
	public NanoRobot(String name, double friction, double positionX,
			double positionY, double diameter, double mass, int model) {
		super(name, friction, positionX, positionY, diameter, mass, model);
	}

	public NanoRobot(String name, double friction, double positionX,
			double positionY, double diameter, double mass) {
	...
	// THe ConfigXML document contains for each muvium project the static
	// configurations information
	// including the classfile to load into the container, the wiring
	 ...
	 * THe procedure is
	 * 
	 * Reset - Resetting the device will instantiate the robot and attempt to
	 * put it into bootmode When using softboot this will happen automatically.
	 * When using hardboot a physical reset is required
	 * 
	 * Program - Once in bootmode the real-robot can be programmed with the
	 * current Agent or if the agent is already the most recent then there is no
	 * need to program it
	 * 
	 ...
	 * 
	 * Presently the jobot simulator is a Real-Real time system which means it
	 * gets it's time step from the actual real-time. The problem with this is
	 * when you have sections of the simulation which take more processing time
	 * to simulate than real time and so the result is large time steps which
	 * are 'jerky' and result in reduced accuracy. Simulated real-time on the
	 * other hand produces a computed time-step. When the simulation runs faster
	 * than real-time it can slow itself down to real-time but when it is slower
	 * then the system remains accurate. At present it is assumed the system
	 * runs in real-time but when this doesn't hold true ie if the
	 * periodicTImers are too fast then the simulation can break down Using the
	 * time-step to synchronise the simulation is the solution to this problem.
	 * 
	 * @param timeStep
	 *            DOCUMENT ME!
	 ...
	 ******************************************************************* 
	 * 
	 * The UVMRobot is the proxy to the remote readSensor webService. THis
	 * allows an external application to invoke the exact same method from this
	 * ProxyRobot using the exact same interface as the one that is actually on
	 * the remote device. This is why it is called a proxy - this interfaced
	 * object for all intents and purposes is a teleporting window to the remote
	...
	public void vector(int vx, int vy, int omega) {
		webServiceV("vector", new Object[] { new Integer(vx), new Integer(vy),
				new Integer(omega) });
	}
	...
	public void drive(int x, int y, int z) {
		webServiceV("drive", new Object[] { new Integer(x), new Integer(y),
				new Integer(z) });
	}
	...
	private int webServiceI(String method, Object[] params) {
		...
		if (icd != null) {
			...
			try {
				return ((Integer) icd.invokeRemoteMethod(method, signature,
						params)).intValue();
			} catch (Exception e) {
				System.out.println(e.toString());
				return -1;
