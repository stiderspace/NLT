...
public class DistanceSensor extends Sensor {
	...
	protected static int keyValuePairs[] = { 0, 151, 1, 160, 2, 190, 3, 215, 4,
			316, 5, 369, 6, 392, 7, 472, 8, 507, 9, 503, 10, 472, 11, 450, 12,
			420, 13, 391, 14, 365, 15, 340, 16, 321, 17, 312, 18, 305, 19, 290,
			20, 270, 25, 220, 30, 178, 35, 149, 40, 126, 45, 110, 50, 98, 55,
			86, 60, 80, 65, 75, 70, 70, 75, 35, // TODO: Check if this
												// sensorvalue
												// is correct for a distance of
												// 75
												// cm
			80, 0 };

	/**
	 * keeps a pre-calculated table of values per 0-80 cm to speed up lookups
	 */
	protected static int[] cmValueCache = loadValueTable();

	/**
	 * Creates a new DistanceSensor object under specified angle and postion
	 */
	public DistanceSensor(String name, Location position, double angle,
			double defaultValue) {
		setName(name);
	...
	/**
	 * @return array of interpolated values to be used as cache. Index is cm,
	 *         value is sensorvalue.
	 */
	protected static int[] loadValueTable() {
		...
		for (int distance = 21; distance < 80; distance++) {
			...
			cmValueCache[distance] = interpolateLinear(
					cmValueCache[lowBoundIndex], lowBoundIndex,
	...
	public static int interpolateLinear(int lowBound, int lowBoundIndex,
			int upBound, int upBoundIndex, int index) {
	...
	/**
	 * Convert a (real) sensor value to a distance reading
	 */
	...
	/*
	 * Note: from 0 - 8 CM the measuring is imprecise.
	 */
