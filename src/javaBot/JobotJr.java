...
public class JobotJr extends Robot {
	 ...
	 *            Name of the JobotJr
	 * @param positionX
	 *            The absolute x-as location in the world
	 * @param positionY
	 *            The absolute y-as location in the world
	 */
	public JobotJr(String name, double positionX, double positionY) {
		super(name, FRICTION, positionX, positionY, BASE_RADIUS * DIAMETER,
				MASS);
		...
		try {
			...
			// direction]
			getSensors()[0] = addSensor("DistanceShortSensor", BASE_RADIUS,
					150, 110); // Left
			getSensors()[1] = addSensor("DistanceShortSensor", BASE_RADIUS, 30,
					70); // Right
			getSensors()[2] = addSensor("FieldSensor", BASE_RADIUS, 130); // Left
																			// /
																			// Middle
