...
public class JoBotProSoccerRobot extends NanoRobot {
	...
	private static final String CONFIG_XML = Simulator
			.getRelativePath("./JobotPro/Soccer/joBotNano.xml");
	 ...
	 */
	public JoBotProSoccerRobot(String name, double positionX, double positionY) {
		/*
		...
		try {
			...
			getSensors()[4] = addSensor("DummySensor", 0, 0);                    // Dummy compass and voltage			getSensors()[4] = addSensor("IRSensor", WHEEL_RADIUS, 240, 250);     // Right
			getSensors()[5] = addSensor("FieldSensor", BASE_RADIUS, 300);        // Right
			getSensors()[6] = addSensor("ContactSensor", BASE_RADIUS, 60, 240);  // Right
			getSensors()[7] = addSensor("CompassSensor", 0, 270);
