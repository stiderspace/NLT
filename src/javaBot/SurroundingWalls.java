...
public class SurroundingWalls extends NonMovableObject {
	...
	public void collideWith(MovableObject object) {
		// methode is copy pasted from old code it'inst conform nature laws at
		// all!
	...
	public double[] giveSensoryInformationTo(Robot r) {
		...
		for (int i = 0; i < r.getSensors().length; i++) {
			if (r.getSensors()[i] instanceof CompassSensor) {
				CompassSensor ds = (CompassSensor) (r.getSensors()[i]);
				Line sl = ds.sensorLine(r.getLocation().getX(), r.getLocation()
						.getY(), r.getOrientation());
			} else
	...
	public void resizeWalls(double newWidth, double newHeight) {
		...
		// this is because these values are passed incorrectly to the
		// constructor.
		double heightInMeters = this.width;
		double widthInMeters = this.height;

		// These values are used to set the LinePane's size.
		// TODO: Warning: heightInPixels DOES use this.height! This code was
		// build
		...
		wall[2] = new Line(new Location(widthInMeters, heightInMeters),
				new Location(0, heightInMeters));
		wall[3] = new Line(new Location(widthInMeters, 0), new Location(
