 ...
 * TODO: extract common Playingfield/RescueField properties into common
 * baseclass
 * 
 * @see javaBot.PlayingField - almost the same
 * 
 */
public class SoccerField extends FieldSensorInformation {
	...
	public SoccerField() {
		...
		Simulator.IMAGE_FILE = Simulator
				.getRelativePath("./resources/soccerfield.png");
		File file = new File(Simulator.IMAGE_FILE);
		try {
			img = ImageIO.read(file);
			Simulator.IMAGE = img;
		} catch (Exception e) {
			Debug.printError("An error occurred in SoccerField: "
					+ e.toString());
		}
	 ...
	 * 
	 * @param object
	 *            The object to not collide with
	 */
	...
	public GraphicalRepresentation createGraphicalRepresentation() {
		return new SoccerFieldGraphicalRepresentation(this);
	}
 ...
 */
class SoccerFieldGraphicalRepresentation extends GraphicalRepresentation {
	Image image = null;
	 ...
	 */
	public SoccerFieldGraphicalRepresentation(PhysicalObject obj) {
		super(obj);
		Simulator.IMAGE_FILE = Simulator
				.getRelativePath("./resources/soccerfield.png");
		setSize(Simulator.meterToPixel(World.WIDTH),
				Simulator.meterToPixel(World.HEIGHT));
		setOpaque(false);
		setVisible(true);
		...
		try {
			...
			System.out.println("Soccer X=" + width + " Y=" + height);
		} catch (Exception e) {
			Debug.printError("Error loading SoccerField image "
					+ Simulator.IMAGE_FILE + ": " + e.toString());
			e.printStackTrace();
		}
	...
	/**
	 * Draw the soccerfield on the graphics object. Will be done 25 times a
	 * second.
	 * 
	 * @param g
