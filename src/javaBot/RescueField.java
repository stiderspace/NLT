...
public class RescueField extends FieldSensorInformation {
	...
	public RescueField() {
		...
		Simulator.IMAGE_FILE = Simulator
				.getRelativePath("./resources/rescuefield.gif");
		File file = new File(Simulator.IMAGE_FILE);
		try {
			img = ImageIO.read(file);
			Simulator.IMAGE = img;
		} catch (Exception e) {
			Debug.printError("An error occurred in RescueField: "
					+ e.toString());
		}
...
class RescueFieldGraphicalRepresentation extends GraphicalRepresentation {
	...
	public RescueFieldGraphicalRepresentation(PhysicalObject obj) {
		...
		try {
			...
			Debug.printError("Error loading rescueField image "
	...
	/**
	 * Draw the rescuefield on the graphics object. Will be done 25 times a
	 * second.
	 * 
	 * @param g
