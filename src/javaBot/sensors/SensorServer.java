...
/**
 * Version 0.0 28-01-2006 Created from reference app This simulator and sensor
 * interface allows the main program to interface directly with a muVium device
 * and read sensors, connected to a microcontroller. All communication is done
 * through a WebService interface, that simplifies the communication to a Remote
 * Procedure Call. The simulator is included to allow generating random values
 * that may be read as if a real device is attached, allowing tests.
 * 
...
public class SensorServer {
	...
	public static final String SIMULATOR_INTPORT = "9000"; // Simulator
															// interface
	 ...
	 * 
	 * @param simulated
	 *            boolean true if sensors are simulated by simulator webservices
	 */
	public SensorServer(boolean simulated) {
		...
		// simulated sensors on localhost port 8300, 8301. Note: Linux ports
		// <1000 are privileged ports
		sensors.addSensor(new SensorDescription(1, "127.0.0.1",
				SIMULATOR_PORT_SENSOR_1, "10.1.1.7", "ISimSensor", "sim"));
		sensors.addSensor(new SensorDescription(2, "127.0.0.1",
				SIMULATOR_PORT_SENSOR_2, "10.1.1.7", "ISimSensor", "sim"));
		// uvm sensors
		sensors.addSensor(new SensorDescription(3, "127.0.0.1",
				SIMULATOR_PORT_SENSOR_3, "10.1.1.10", "IUvmSensor", "uvm"));
		simData = new SensorDataGetter(sensors);

		if (!sim) {
			addRealMuviumRPCServer(sensors);
		} else {
			// Create the WebService enabled Simulator on 10.1.1.1 port Port80
			// gateway
			RPCServer simulatorServer = new RPCServer(SIMULATOR_PORT);
			simulatorServer.addWebService(SIMULATOR_IP, simData);
			createSimulatedSensor(sensors, SIMULATOR_PORT_SENSOR_1); // Add two
																		// simulated
																		// sensors
	...
	private static void addRealMuviumRPCServer(SensorMap sensorMap) {
		try {
			WebRequest.setSocketImplFactory(new SlipSocketFactory(1, 57600,
					true));
		} catch (Exception e) {
		}
		...
		// You can setup multiple addresses, for example you can have a network
		// of muvium devices
		// programmed with different addresses listening to a multi-drop
		// serial/radio/infrared network
		// WebServices is request/respond so there are no collisions.
		// sensorMap.activateSensors("127.0.0.1", "101");
		// realUVMServer.addWebService( "10.1.1.10" ); //Add a forwarding
		// service for 10.1.1.10
		// realUVMServer.start();
	}
	...
	private static void createSimulatedSensor(SensorMap sensorMap, String port) {
		...
		for (Enumeration e = sensors.elements(); e.hasMoreElements();) {
			...
			sensorPort.addWebService(sensor.getIpAddress(), sensor
					.createService(SIMULATOR_PROXY_IP, SIMULATOR_PORT,
							SIMULATOR_IP));
		}
