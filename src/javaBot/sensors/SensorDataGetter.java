...
public class SensorDataGetter extends Thread implements ISensorSim {
	 ...
	 * 
	 * @param sensorMap
	 *            the sensorMap with all sensors
	 * @see SensorMap
	 */
	...
	public byte[] getSensorData(int sensorId, int type) {
		...
		if (sensor.active) {
			...
			if (sensorProxy instanceof ISensorSimProxy) {
				...
				System.out.println("Getting Simulated Sensor:"
						+ sensor.myDeviceIp + ":" + sensor.myGatewayPort + " ("
						+ sensor.myId + ")=" + value);
	...
	/**
	 * Enumerate the sensormap, extract data from all active sensors and discard
	 * this data
	 * 
	 * @param type
	 *            some sensortype value (?)
	 */
	public void scanSensors(int type) {
		for (Enumeration e = mySensorMap.getSensors().elements(); e
				.hasMoreElements();) {
