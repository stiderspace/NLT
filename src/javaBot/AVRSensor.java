...
/**
 * Used as a simulation of the AVR Camera. It uses two IR sensors to calculate
 * the ball position within the field. The distance and opening angle must be
 * made equal to that of the camera.
 */
public class AVRSensor extends Sensor {
	...
	public double checkFOV(Location robot, double rotationRobot, Location obj) {
		...
		Location point = new Location(robot.getX()
				- (Math.cos(l.getAngle() + rotationRobot) * l.getLength()),
				robot.getY()
						- (Math.sin(l.getAngle() + rotationRobot) * l
								.getLength()));
		...
		double angularDifference = Math.abs(Math.IEEEremainder(angleToObject
				- angleSensor, 2 * Math.PI));

		// Debug.printDebug("angular difference " + angularDifference);
		if (angularDifference <= (fieldOfView / 2)) {
			if (l.getLength() <= maxDist) {
				// Return a value that is dependent on the location of the
				// object
