...
public class World {
	 ...
	 * 
	 * @param gui
	 *            the gui we want everything to be displayed in!
	 */
	World(Simulator gui) {
		Debug.printDebug("Creating world with default settings");
		this.gui = gui;

		NonMovableObject surroundingWall = new SurroundingWalls("walls", WIDTH,
				HEIGHT);
	 ...
	 * 
	 * @param object
	 *            should be completly instantiated
	 */
	 ...
	 * 
	 * @param object
	 *            should be completely instantiated
	 */
	...
	public void handleCollisions() {
		...
		for (int i = 0; i < movableObjects.size(); i++) {
			...
			// We start with j=i+1 to prevent to compare the object with it's
			// self and to prevent us from compare the same object twice
	 ...
	 */
	private void collide(MovableObject movableObject1,
			MovableObject movableObject2) {
		double distance = movableObject1.getLocation().distanceTo(
				movableObject2.getLocation());
		...
		if (distance < minimalDistance) {
			...
			// code from previous year no documentation availeble, it calculates
			...
			// double dVxR1 = movableObject1.getVelocityX() -
			// movableObject2.getVelocityX();
			// double dVyR1 = movableObject1.getVelocityY() -
			// movableObject2.getVelocityY();
			//
			// double dVxR2 = movableObject2.getVelocityX() -
			// movableObject1.getVelocityX();
			// double dVyR2 = movableObject2.getVelocityY() -
			// movableObject1.getVelocityY();
			//
			// double arbeidX;
			// double arbeidY;
			...
			if (((dPxR1 > 0) && (movableObject1.getVelocityX() > 0))
					|| ((dPxR1 < 0) && (movableObject1.getVelocityX() < 0))) { // R1
																				// arbeid
																				// on
																				// R2
			...
			if (((dPxR2 > 0) && (movableObject2.getVelocityX() > 0))
					|| ((dPxR2 < 0) && (movableObject2.getVelocityX() < 0))) { // R2
																				// arbeid
																				// on
																				// R1
			...
			if (((dPyR1 > 0) && (movableObject1.getVelocityY() > 0))
					|| ((dPyR1 < 0) && (movableObject1.getVelocityY() < 0))) { // R1
																				// arbeid
																				// on
																				// R2
			...
			if (((dPyR2 > 0) && (movableObject2.getVelocityY() > 0))
					|| ((dPyR2 < 0) && (movableObject2.getVelocityY() < 0))) { // R2
																				// arbeid
																				// on
																				// R1
	 ...
	 * 
	 * @param robot
	 *            robot we want to let know what his sensor readings are
	 */
	private void updateSensors(Robot robot) {
		// here we store the current object we're getting sensor information
		// from
		...
		// if the sensors registered something other than 0, overwrite current
		// sensor values
		for (int o = 0; o < robot.getSensors().length; o++) {
			Sensor s = robot.getSensors()[o];
				s.setValue(sensorValues[o]);
		}
	}
	...
	public void resize(double width, double height) {
		...
		ratioHeight = (double) gui.getWorldPane().getHeight()
				/ Simulator.DEFAULT_WORLD_PANE_HEIGHT;
		double newPixelsPerMeter = (Simulator.DEFAULT_WORLD_PANE_HEIGHT / height)
				* ratioHeight;
		...
		if (getRescueField() != null) {
			...
			getRescueField().rescaleImage((int) (width * newPixelsPerMeter),
					(int) (height * newPixelsPerMeter));
		}
		if (getPlayingfield() != null) {
			resizeField(getPlayingfield().getGraphicalRepresentation());
			getPlayingfield().rescaleImage((int) (width * newPixelsPerMeter),
					(int) (height * newPixelsPerMeter));
		}
		if (getDanceFloor() != null) {
			resizeField(getDanceFloor().getGraphicalRepresentation());
			getDanceFloor().rescaleImage((int) (width * newPixelsPerMeter),
					(int) (height * newPixelsPerMeter));
		}
	...
	public void resizeField(GraphicalRepresentation graphicalRepresentation) {
		graphicalRepresentation
				.setWidth((int) (World.WIDTH * Simulator.pixelsPerMeter));
		graphicalRepresentation
				.setHeight((int) (World.HEIGHT * Simulator.pixelsPerMeter));
