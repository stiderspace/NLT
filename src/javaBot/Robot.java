...
public abstract class Robot extends MovableObject implements IDragable {
	 ...
	 * @param mass
	 */
	protected Robot(String name, double friction, double positionX,
	 ...
	 */
	protected Sensor addSensor(String sensorClassName, double radius,
			double position) throws ReflectionException {
		return addSensor(sensorClassName, radius, position, position);
	}

	protected Sensor addSensor(String sensorClassName, double radius,
			double position, double direction) throws ReflectionException {
		// convert degrees to radians
		double radianPosition = degreesToRadians(position);
		double radianDirection = degreesToRadians(direction);

		// position of the sensor
		Location location = new Location(radius * Math.cos(radianPosition),
		...
		try {
			...
			Class[] classParm = new Class[] {
					Class.forName(PACKAGE_NAME + ".Location"), Double.TYPE };
			Object[] objectParm = new Object[] { location,
					new Double(radianDirection) };
	...
	public void update(double elapsed) {
		...
		/*
		 * A check if the velocity equals 0 is being made to prevent the dvx and
		 * dvy values to be set to NaN, due to a division by zero, when it
		 * should be 0.0.
		 */
		...
		/*
		 * Do not allow the direction of the velocity to change, because of
		 * sampling
		 */
	...
	/*
	 * The option "Field Boundaries" in the menu controls the behaviour of the
	 * robot when the object is about to pass the surrounding walls. The robot
	 * will either be put back inside the field or leave the field to enter it
	 * on the opposite side.
	 * 
	 * The ROBOT_SENSOR_OFFSET is the distance between the part of the robot
	 * that actually hits the wall and the sensor.
	 */
	protected double[] getCoordinatesAccordingToBoundary(double xCoordinate,
			double yCoordinate) {
