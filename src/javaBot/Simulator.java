...
import javaBot.agents.Swarming;
 ...
 * Ver 4.0 - 18-07-2012 - Included JoBot Pro and Swarming
 */
...
public class Simulator extends JPanel {
	...
	private static final String WINDOW_TITLE = "NLT Student V400";
	private static final Color GRID_COLOR = new Color(0.9f, 0.9f, 0.9f);
	...
	private static final String SHOW_SOCCERFIELDSGL = "Show soccerfield (single)";
	private JRadioButtonMenuItem showSoccerfieldSgl = new JRadioButtonMenuItem(
			SHOW_SOCCERFIELDSGL);
	...
	private static final String SHOW_SOCCERFIELDJNR = "Show soccerfield (junior)";
	private JRadioButtonMenuItem showSoccerfieldJnr = new JRadioButtonMenuItem(
			SHOW_SOCCERFIELDJNR);
	...
	private JCheckBoxMenuItem pause = new JCheckBoxMenuItem("Pause");
	...
	private JCheckBoxMenuItem showGrid = new JCheckBoxMenuItem(SHOW_GRID);

	private static final String SHOW_LABELS = "Show labels";
	private JCheckBoxMenuItem showLabels = new JCheckBoxMenuItem(SHOW_LABELS);
	...
	public void run() {
		...
		DisplayMode scr = frame.getGraphicsConfiguration().getDevice()
				.getDisplayMode();
		frame.setLocation((scr.getWidth() - frame.getWidth()) / 2,
				(scr.getHeight() - frame.getHeight()) / 2);
	...
	public static void main(String[] args) {
		...
		if (args.length == 1) {
			...
			System.out
					.println("Usage: java Simulator [single/double]\nUsing default field SIZE");
		}
	...
	public void update() {
		...
		for (int i = 0; i < nonMovableObjects.size(); i++) {
			NonMovableObject object = (NonMovableObject) nonMovableObjects
					.get(i);
			component = object.getGraphicalRepresentation();
			if (component != null) {
				// Add the MouseListener if not done already
	...
	public void printMessage(String s) {
		...
		if (lines > MESSAGE_PANE_MAXLINES) {
			messageTextArea.setText("NOTE: Output buffer cleaned up after "
					+ MESSAGE_PANE_MAXLINES + " lines\n");
			return;
		}

		if (autoScrollState) {
			messageTextArea.getCaret().setDot(
	...
	private JLabel getWarningButton() {
		...
		speedSliderWarningLabel.setIcon(new ImageIcon(
				SPEED_SLIDER_WARNING_IMAGE));
	...
	private JMenu getMenuSimulation() {
		...
		menuSimulation.addSeparator();
		// Rest weg voor leerlingen versie
//		menuSimulation.add(new JMenuItem("BallLover vs. BallLover"));
//		menuSimulation.add(new JMenuItem("Swarming"));
//		menuSimulation.add(new JMenuItem("SoccerDemo"));
//		menuSimulation.add(new JMenuItem("JobotSoccerDemo"));
//		menuSimulation.addSeparator();
		...
		menuSimulation.addSeparator();
		// Rest weg voor leerlingen versie
//		menuSimulation.add(new JMenuItem("Test - Nano"));
//		menuSimulation.add(new JMenuItem("Soccer - Nano"));
//		menuSimulation.addSeparator();
//		menuSimulation.add(new JMenuItem("Grid - Junior"));
//		menuSimulation.add(new JMenuItem("Rescue - Junior"));
//		menuSimulation.add(new JMenuItem("Soccer - Junior"));
//		menuSimulation.add(new JMenuItem("Test - Junior"));
//		menuSimulation.addSeparator();
//		menuSimulation.add(new JMenuItem("Grid - Jobot"));
//		menuSimulation.add(new JMenuItem("Test - Jobot"));
//		menuSimulation.add(new JMenuItem("Rescue - Jobot"));
//		menuSimulation.add(new JMenuItem("Soccer - Jobot"));
		// Tot hier weg voor leerlingen versie
		menuSimulation.addSeparator();
	...
	private JMenu getMenuInsert() {
		...
		submenuObject.add(new JMenuItem("Can"));
		...
		submenuRobot.add(new JMenuItem("JoBot Nano"));    // Leerlingen versie
//		submenuRobot.add(new JMenuItem("JoBot Junior"));
//		submenuRobot.add(new JMenuItem("JoBot Pro"));
//		submenuRobot.add(new JMenuItem("JoBot"));
		submenuRobot.add(new JMenuItem("NXT"));           // NXT experimenteel
//		submenuRobot.add(new JMenuItem("NXT Rescue"));
	...
	/**
	 * Subclasses should be generic reusable object. This is not (Sorry can't
	 * help this code stinks).
	 * 
	 * This class is only used to override the paintComponent(Graphics g)
	 * method. The only reason this class exists is that it's better then using
	...
	private class WorldPane extends JLayeredPane {
		...
		public void paintComponent(Graphics g) {
			...
			if (grid && (worldPane != null)) {
				...
				/*
				 * Grid starts in the top left corner instead of world
				 * coordinates (0,0) because Maze does so too.
				 */
	...
	public RobotGUI getRobotGUI() {
		...
		if (selectedObject instanceof Robot) {
			if (guiRegistry.containsKey(selectedObject)) {
				...
				RobotGUI robotGUI = new RobotGUI((Robot) selectedObject,
						getSimulator());
	...
	private void clearIDragableObjectsfromWorld() {
		...
		for (int i = 0; i < nonMovableobSize; i++) {
			if (nonMovableObjects.get(i) instanceof IDragable) {
				world.removeNonMovableObject((NonMovableObject) nonMovableObjects
						.get(i));
			}
		}
	}
	 ...
	 * @version $Revision$ last changed 20-02-2006
	 */
	private class SimulatorActionListener implements ActionListener,
			ChangeListener {
		/*
		 * (non-Javadoc)
		 * 
...
//========= Robots ================
			...
			if (e.getActionCommand().equals("JoBot Nano")) {
				Robot r = new JoBotNanoNLTGridRobot("JobotNano", World.WIDTH / 2,
						World.HEIGHT / 2);
				addRobotToGui(r);
			...
			if (e.getActionCommand().equals("JoBot Junior")) {
				Robot r = new JPB2JrNLTGridRobot("JobotJr", World.WIDTH / 2,
						World.HEIGHT / 2);
				addRobotToGui(r);
			...
			if (e.getActionCommand().equals("JoBot")) {
				Robot r = new JPB2GridRobot("Jobot", World.WIDTH / 2,
						World.HEIGHT / 2);
				addRobotToGui(r);
			...
			if (e.getActionCommand().equals("JoBot Pro")) {
				Robot r = new JoBotProGridRobot("JobotPro", World.WIDTH / 2,
						World.HEIGHT / 2);
				addRobotToGui(r);
			...
			if (e.getActionCommand().equals("NXT")) {
				Robot r = new NXTGridRobot("NXT", World.WIDTH / 2,
						World.HEIGHT / 2);
				addRobotToGui(r);
			...
			if (e.getActionCommand().equals("NXT Rescue")) {
				Robot r = new NXTRescueRobot("NXT Rescue", World.WIDTH / 2,
						World.HEIGHT / 2);
				addRobotToGui(r);
			...
			if (e.getActionCommand().equals("RCJoBot")) {
				Robot r = new RCJobot("RCJobot", World.WIDTH / 2,
						World.HEIGHT / 2);
				addRobotToGui(r);
			...
			if (e.getActionCommand().equals("SwarmBot")) {
				Robot r = new SwarmBot();
				addRobotToGui(r);
...
//========== Demo modules ================
		...
		if (e.getActionCommand().equals("RescueDemo"))
			{
				...
				Can c = new Can();
				world.addMovableObject(c);
				c.setLocation(new Location(0.4, World.HEIGHT - 0.4));
		...
		if (e.getActionCommand().equals("Balls and Followers")) {
				...
				showSoccerfieldJnr.setSelected(false);
				showEmptyfield.setSelected(true);
				...
				for (int i = 0; i < 10; i++) {
					...
					// Ideally you want to change the diameter of the robots, to
					// make
					// the demo nicer.. though the collision detection in
					// SoccerWalls.java
					// is not compatible with diameters below 0.10!!! Hence the
					// following
					...
					j.mass = 1;
					j.setLocation(new Location(Math.random() * World.WIDTH,
	...
	// ===== Vanaf hier ongebruikt in leerlingen versie ========

			if (e.getActionCommand().equals("Swarming")) {
				clearMovableObjectsfromWorld();
				showRescuefield.setSelected(false);
				showSoccerfieldJnr.setSelected(false);
				showEmptyfield.setSelected(true);
				...
				// Create some  robots
				for (int i = 0; i < 10; i++) {
					SwarmBot j = new SwarmBot();
					// j.diameter = 0.05;
					j.mass = 1;
					j.setLocation(new Location(Math.random() * World.WIDTH,
							Math.random() * World.HEIGHT));
					j.name = "B" + i;
					new Thread(new Swarming(j)).start();
					world.addMovableObject(j);
//					j.setColor(Color.MAGENTA);
			...
			if (e.getActionCommand().equals("SoccerDemo")) {
				clearMovableObjectsfromWorld();
				showSoccerfieldDbl.setSelected(true);
				// world.resize(4.0, 3.0);
				world.resize(6.0, 4.5);

				// Create two robot teams
				for (int i = 0; i < 5; i++) {
					Robot j = new JPBSoccerRobot("SoccerDemo", 0.15,
							World.HEIGHT / 2);
					j.diameter = 0.18;
					j.mass = 1;
					j.setLocation(new Location(Math.random() * World.WIDTH,
					...
					j.name = "A" + i;
					world.addMovableObject(j);
					RobotGUI robotGUI = new RobotGUI(j, getSimulator());
					robotGUI.setDipSwitch(2);
				}
				for (int i = 0; i < 5; i++) {
					Robot j = new JPBSoccerRobotB("SoccerDemo", 0.15,
							World.HEIGHT / 2);
					j.diameter = 0.18;
					j.mass = 1;
					j.setLocation(new Location(Math.random() * World.WIDTH,
			...
			if (e.getActionCommand().equals("JrSoccerDemo")) {
				clearMovableObjectsfromWorld();
				showSoccerfieldSgl.setSelected(true);
			...
			if (e.getActionCommand().equals("JobotSoccerDemo")) {
				clearMovableObjectsfromWorld();
				showSoccerfieldDbl.setSelected(true);
				Robot r = new JPBSoccerRobot("JobotSoccerDemo", 0.15, World.HEIGHT / 2);
				r.diameter=0.12;
	...
	// NLT Lessen -------------------------------------------------------
			...
			if (e.getActionCommand().equals("Grid - Nano"))
			{
				...
				showRescuefield.setSelected(false);
				showSoccerfieldSgl.setSelected(false);
				showSoccerfieldDbl.setSelected(false);
				showEmptyfield.setSelected(true);
...
//					robotGUI.setDipSwitch(2);
			...
			if (e.getActionCommand().equals("BallLover vs. BallLover")) {
				clearMovableObjectsfromWorld();
				showSoccerfieldJnr.setSelected(true);
...
// ========== Einde menu items NLT =================
			...
			if (e.getActionCommand().equals("pauze")
	...
	private class SimulatorItemListener implements ItemListener {
		...
		public void itemStateChanged(ItemEvent e) {
			if ((e.getItem() instanceof JCheckBoxMenuItem)) {
				...
				if (text.equals(SHOW_EMPTY_FIELD)) {
					...
					showDanceFloor();
				} else if (text.equals(SHOW_SOCCERFIELDSGL)) {
					showSoccerField(0);
				} else if (text.equals(SHOW_SOCCERFIELDDBL)) {
					showSoccerField(1);
				} else if (text.equals(SHOW_SOCCERFIELDJNR)) {
					showSoccerField(2);
				}
			} else {
				System.out.println("Current button is not supported"
						+ e.getItem().getClass());
			}
		}
		...
		public void doCBItemChange(JCheckBoxMenuItem item, boolean state) {
			...
			if (text.equals("Pause")) {
				if (state == true) {
					...
					Debug.printDebug("Time elapsed since previous update is "
							+ previousFrame + " ms");
				} else {
					/*
			...
			// {
			// ((Robot)
			// movableObjects.get(i)).setCollideWithBoundaries(fieldBoundaries
			// .getState());
			// }
			// }
			// }
				...
				if (!autoScrollState && messageTextArea.getText().length() > 0)
					messageTextArea.getCaret().setDot(
							messageTextArea.getText().length() - 1);
		 ...
		 * 
		 */
		private void showSoccerField(int type) {
			if (type == 1) {
				if (world.getSoccerField() == null) {
					world.setSoccerField(new SoccerField());
					world.addNonMovableObject(world.getSoccerField());
				} else {
					world.removeNonMovableObject(world.getSoccerField());
					world.setSoccerField(null);
				}
				...
				if (type == 2) {
					...
					NonMovableObject socWalls = new SoccerWalls("soccerwall",
							World.HEIGHT);
			...
			if (type == 0)
				...
				world.resize(2.00, 1.50);
			else if (type == 2)
				world.resize(4.00, 3.00);
			return;
		}
	 ...
	 * @version $Revision$ last changed 20-02-2006
	 */
	private class SimulatorMouseListener implements MouseListener,
			MouseMotionListener {
		/**
		...
		public void mousePressed(MouseEvent mouseEvent) {
			...
			selectedObject = ((GraphicalRepresentation) component)
					.getPhysicalObject();
			...
			if ((mouseEvent.getButton() == RIGHT_MOUSE_BUTTON)
				...
				if (selectedObject instanceof Robot) {
					...
					jpopRobot.show(component, mouseEvent.getX(),
							mouseEvent.getY());
				} else {
					// no robot, so show non-robot pulldown at mouse coordinates
					jpopNonRobot.show(component, mouseEvent.getX(),
							mouseEvent.getY());
				}
		...
		private void handleLeftMouseButton(MouseEvent mouseEvent,
				Component component) {
			selectedObject = ((GraphicalRepresentation) component)
					.getPhysicalObject();
			...
			if (selectedObject instanceof IDragable) {
				if (mouseEvent.isShiftDown()) {
					...
					double dx = mouseEvent.getX()
							- ((GraphicalRepresentation) component)
									.getOffsetX();
		...
		public void mouseDragged(MouseEvent mouseEvent) {
			...
			// the current frame, otherwise the event is fired way too often
			if (mouseFrameDone == false) {
				...
				case ROTATE_MOUSE_ACTION:
					...
					double dx = mouseEvent.getX()
							- ((GraphicalRepresentation) (mouseEvent
									.getComponent())).getOffsetX();
					double dy = -mouseEvent.getY()
							+ ((GraphicalRepresentation) (mouseEvent
									.getComponent())).getOffsetY();
					selectedObject.setRotation(Math.atan2(dy, dx) - initialRotation);
//					System.out.println("Rotation " + selectedObject.getRotation());
				...
				case MOVE_MOUSE_ACTION:
					...
					double newY = l.getY()
							- (mouseEvent.getY() / pixelsPerMeter)
							+ (selectedObject.diameter / 2);
				...
				case VELOCITY_MOUSE_ACTION:
					...
					if (selectedObject instanceof MovableObject) {
						((MovableObject) selectedObject)
								.setVelocityX(mouseEvent.getX()
										/ pixelsPerMeter);
						((MovableObject) selectedObject)
								.setVelocityY(-mouseEvent.getY()
										/ pixelsPerMeter);
					}
	...
	/**
	 * This class is used as workaround for the console scrolling bug This bug
	 * is only present in JDK 1.5 on Linux (X11)
	 */
	private class ConsoleCaret extends DefaultCaret {

		/**
		 * If the field is invalid when setting the mouse cursor revalidate and
		 * try again.
		 */
