...
public final class FastFourierTransform {
	...
	/**
	 * Method for converting the output of the sensor to a real byte array. The
	 * sensor uses char but tje interface outputs a byte array. This causes a
	 * problem with the sign bit because chars run from 0..255 and byte from
	 * -128..127. These need to be converted. So char value 0..128 is converted
	 * to the range -128..0 And char value 129..255 is converted to the range
	 * 0..127
	 */
	 ...
	 * 
	 * @param originalData
	 *            Contains the data which will be analysed. The length must be a
	 *            power of 2.
	 */
	public final byte[] fftMag(byte[] originalData) {
		...
		resultingVector[0] = (float) (Math.sqrt(realNumbers[0] * realNumbers[0]
				+ imaginairNumbers[0] * imaginairNumbers[0]))
				/ n;
		for (int i = 1; i < n / 2; i++) {
			resultingVector[i] = 2
					* (float) (Math.sqrt(realNumbers[i] * realNumbers[i]
							+ imaginairNumbers[i] * imaginairNumbers[i])) / n;
