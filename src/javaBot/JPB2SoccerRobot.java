 ...
 * with electical connections provided as per the JPB specification. The
 * UVMRobot can also act as an avatar for a real robot. In particular the
 ...
 * 
 * @author James Caska The UVMRobot proxy implements the [JC] September 15, 2005
 *         Seperated JPB2Robot into implementation of UVMRobot which gives
 *         better decoupling of the services available to each UVMRobot and the
 *         specific robot implementation provided by the instance of the
 *         JPB2Robot
 */
public class JPB2SoccerRobot extends UVMRobot {
	...
	private static final String CONFIG_XML = Simulator
			.getRelativePath("./JPB2/Soccer/jobot.xml");
	 ...
	 * @param positionY
	 */
	public JPB2SoccerRobot(String name, double positionX, double positionY) {
		/*
		...
		try {
			...
			getSensors()[4] = addSensor("FieldSensor", BASE_RADIUS, 60); // Left
			getSensors()[5] = addSensor("IRSensor", WHEEL_RADIUS, 300, 290);
			getSensors()[6] = addSensor("IRSensor", WHEEL_RADIUS, 240, 250);
																			// /																		// Middle
			getSensors()[4] = addSensor("FieldSensor", BASE_RADIUS, 120); // Right
			// getSensors()[5] = addSensor("IRSensor", BASE_RADIUS, 410, 270);
			// // Left
			// getSensors()[6] = addSensor("IRSensor", BASE_RADIUS, 480, 270);
			// // Right
		}
...
//		System.out.println("Chan=" + channel + ",Val="+ val);
	...
	public void loadApp(String configXML) {
		 ...
		 */
		servoMotors = new ServoMotor[3];

		for (int i = 0; i < servoMotors.length; i++) {
			servoMotors[i] = new ServoMotor(0.0, 156,
					1 * Units.MILLISECONDS, 2 * Units.MILLISECONDS, "S3003");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see javaBot.MovableObject#update(double) Please note that the standard
	 * futaba servo's rotate in the opposite direction compared to the HiTec
	 * servos.
	 */
