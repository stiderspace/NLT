 ...
 * it so the robot can determine it's approximate location using the FieldSensor
 * 
...
public class PlayingField extends FieldSensorInformation {
	...
	public PlayingField() {
		...
		Simulator.IMAGE_FILE = Simulator
				.getRelativePath("./resources/field.gif");
		File file = new File(Simulator.IMAGE_FILE);
		try {
			img = ImageIO.read(file);
			Simulator.IMAGE = img;
		} catch (Exception e) {
			Debug.printError("An error occurred in PlayingField: "
					+ e.toString());
		}
...
class PlayingFieldGraphicalRepresentation extends GraphicalRepresentation {
	...
	public PlayingFieldGraphicalRepresentation(PhysicalObject obj) {
		...
		try {
			...
			Debug.printError("Error loading playingField image "
	...
	/**
	 * Draw the playingfield on the graphics object. Will be done 25 times a
	 * second.
	 * 
	 * @param g
