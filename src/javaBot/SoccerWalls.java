...
public class SoccerWalls extends NonMovableObject {
	...
	public void resizeWalls() {
		...
		// walls
		wall[0] = new Line(new Location(rightside, 0), new Location(rightside,
				nokeep));
		wall[1] = new Line(new Location(rightside + corner, nokeep),
				new Location(rightside + corner, height - nokeep));
		wall[2] = new Line(new Location(rightside, height - nokeep),
				new Location(rightside, height));
		wall[3] = new Line(new Location(leftside, height), new Location(
				leftside, height - nokeep));
		wall[4] = new Line(new Location(leftside - corner, height - nokeep),
				new Location(leftside - corner, nokeep));
		wall[5] = new Line(new Location(leftside, nokeep), new Location(
				leftside, 0));

		// horizontal pieces
		wall[6] = new Line(new Location(leftside - corner, nokeep),
				new Location(leftside, nokeep));
		wall[7] = new Line(new Location(leftside, height - nokeep),
				new Location(leftside - corner, height - nokeep));
		wall[8] = new Line(new Location(rightside, nokeep), new Location(
				rightside + corner, nokeep));
		wall[9] = new Line(new Location(rightside + corner, height - nokeep),
				new Location(rightside, height - nokeep));

		// diagonal corners
		wall[10] = new Line(new Location(leftside, corner), new Location(
				leftside + corner, 0));
		wall[11] = new Line(new Location(rightside - corner, 0), new Location(
				rightside, corner));
		wall[12] = new Line(new Location(rightside, height - corner),
				new Location(rightside - corner, height));
		wall[13] = new Line(new Location(leftside + corner, height),
				new Location(leftside, height - corner));

		// Top and Bottom line
		wall[14] = new Line(new Location(0, height), new Location(World.WIDTH,
				height));
	...
	public void collideWith(MovableObject object) {
		for (int i = 0; i < wall.length; i++) {
			...
			if ((dist < rad) || (isObjectOutField(object, wall[i], i, rad))) {
				...
				object.setLocation(new Location(pos.getX()
						+ (rad * Math.cos(coef)), pos.getY()
						+ (rad * Math.sin(coef))));
				...
				if (i < 6) {
					...
					object.setVelocityY(object.getVelocityX()
							* elasticityFactor);
				...
				object.setAccelerationX(object.getAccelerationX()
						* -elasticityFactor);
				break;
			}
		}
	...
	/**
	 * @param object
	 *            Ball, Robot, every object who can not be out of the
	 *            soccerfield
	 * @param currentWall
	 * @param lineNumber
	 *            The number of wall, can be used to view which type of wall
	 *            this is.
	 * @param objectRadius
	 *            Radius of object
	 * @return true if ball is no longer in the soccer field
	 */
	public boolean isObjectOutField(MovableObject object, Line currentWall,
			int lineNumber, double objectRadius) {
		double objectX = object.getLocation().getX();
		...
		case 7:
		case 9:
			return (objectY > wallEndY)
					&& ((wallStartX > objectX) && (wallEndX < objectX));
			...
			if (r.getSensors()[i] instanceof DistanceSensor) {
				...
				for (int wallCounter = 0; wallCounter < wall.length; wallCounter++) {
					Line sl = ds.sensorLine(r.getLocation().getX(), r
							.getLocation().getY(), r.rotation);
					double d = ds.convertDistanceToValue(sl
							.intersectDist(wall[wallCounter]));
