...
public class Jobot extends Robot {
	...
	public String IMAGE_FILE = Simulator.getRelativePath("./resources/jobot.gif");
	private static final int DIAMETER_IN_IMAGE = 100;

	/**
	 * Creates a new Jobot with 5 init values:
	 * 
	 * friction = 7.0 positionX = World.GRID_SIZE / 2 positionY = World.HEIGHT -
	 * (World.GRID_SIZE / 2) diameter = 2.0 mass = 3.0
	 * 
	 */
	
	public Jobot() {
		super("Jobot", 7.0, World.GRID_SIZE / 2, World.HEIGHT
				- (World.GRID_SIZE / 2), 2 * BASE_RADIUS, 2);
		init();
	}

	/**
	 * Creates a new Jobot object. Initializes 3 (default)values: friction = 7.0
	 * diameter = 2.0 mass = 3.0
	 * 
	 * @param name
	 *            name of the robot
	 * @param positionX
	 *            X-position where the robot's need to be located
	 * @param positionY
	 *            Y-position where the robot's need to be located
	 * 
	 * 
	 */
	...
	/**
	 * This method is called by the constructor and creates the wheels / sensors
	 * / leds
	 * 
	 */
	private void init() {
		...
		double[] wheelAngles = { (6 / 6.) * Math.PI, (10 / 6.) * Math.PI,
				(2 / 6.) * Math.PI };
	 ...
	 * the MovableObject with. It's location within the GUI in pixel values will
	 * be set by the GUI itself.
	 * 
	...
	public void update(double elapsed) {
		...
		if ((getActors() != null) && (getActors().length == 3)) {
			...
			setDrivingVelocityY((WHEEL_RADIUS * (((fx[1] - fx[2]) * w[0])
					...
					/ -div);
			this.setRotationSpeed((WHEEL_RADIUS * ((((-fx[2] * fy[1]) + (fx[1] * fy[2])) * w[0])
