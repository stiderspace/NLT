...
public class Line {
	...
	public Location intersect(Line b) {
		Location intsec = new Location(
				((p.getX() * Math.cos(b.getAngle()) * Math.sin(getAngle())) - (Math.cos(getAngle()) * (((p.getY() - b.p.getY()) * Math.cos(b
						.getAngle())) + (b.p.getX() * Math.sin(b.getAngle())))))
						/ Math.sin(getAngle() - b.getAngle()),
				((b.p.getY() * Math.cos(b.getAngle()) * Math.sin(getAngle())) - (((p
						.getY() * Math.cos(getAngle())) + ((b.p.getX() - p
						.getX()) * Math.sin(getAngle()))) * Math.sin(b
						.getAngle())))
						/ Math.sin(getAngle() - b.getAngle()));
	...
	public boolean checkLocationInLine(Location l) {
		if (ignoreLineEnds == false) {
			...
			// Debug.printDebug(minX + " " + minY + " " + maxX + " " + maxY +
			// " " + l.toString() + " " + this.toString());
	...
	/**
	 * Calculate the distance from a line through one point to the intersection
	 * with another line
	 * 
	 * @param b
	 *            Line to intersect with
	...
	public double intersectDist(Line b) {
		...
		// Debug.printDebug(toString() + " " + b.toString() +
		// " Found intsec at " + intsec.toString());
		// Check if the intersection is in fact within the length of both lines
		if (!b.checkLocationInLine(intsec)) {
			return Double.POSITIVE_INFINITY;
		}

		return -(((p.getY() - b.p.getY()) * Math.cos(b.getAngle())) + ((-p
				.getX() + b.p.getX()) * Math.sin(b.getAngle())))
				/ Math.sin(getAngle() - b.getAngle());
	}
	...
	public double intersectDist2(Line b) {
		...
		return Math.sqrt(((p.getY() - intsec.getY()) * (p.getY() - intsec
				.getY()))
	...
	public String toString() {
		return "Line of " + getLength() + " meters through " + p.toString()
				+ " at an angle of " + (getAngle() / Math.PI) + " PI to "
				+ q.toString();
	}
	 ...
	 * DOCUMENT ME!
	 * 
	 * @param ignoreLineEnds
	 *            The ignoreLineEnds to set.
	 */
