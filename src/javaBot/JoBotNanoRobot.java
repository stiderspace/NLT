...
public class JoBotNanoRobot extends NanoRobot {
	...
	public JoBotNanoRobot(String name, double positionX, double positionY) {
		...
		try {
			...
			getSensors()[1] = addSensor("FieldSensor", BASE_RADIUS, 110); // Left
			...
			getSensors()[5] = addSensor("FieldSensor", BASE_RADIUS, 70); // Right
...
//		System.out.println("Loading from config file: " + configXML);
		 ...
		 */
		servoMotors = new ServoMotor[2];

		for (int i = 0; i < servoMotors.length; i++) {
			servoMotors[i] = new ServoMotor(0.0, 156,
					1 * Units.MILLISECONDS, 2 * Units.MILLISECONDS, "S148");
