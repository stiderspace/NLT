...
/**
 * The RobotGUI has a Frame that shows various info items about the status of a
 * robot and allows for interaction with the robot line selecting an agent,
 * changing dipswitches and sending commands.
 */
public class RobotGUI implements IGUI {
	...
	/**
	 * This function sets a dipSwitch and executes the executeDipSwitch function
	 * 
	 * @param dipSwitch
	 ...
	 *            int command
	 * @param param0
	 *            int the command parameter
	 */
	private void doCommand(int command, int param0) {
		doCommand(command, param0, UVMRobot.COMMAND_NULL_PARAMETER,
				UVMRobot.COMMAND_NULL_PARAMETER);
	}
	...
	/**
	 * Called from the actionlistener to send the text the user typed to the
	 * Robot using doCommand(). Will only work on UVMRobots
	 * 
	 * @see RobotGUI#doCommand(int, int, int, int)
	 * 
	 */
	private void doSendCommand() {
		...
		if (robot instanceof UVMRobot) {
			...
			if (cmd.length == 1) {
				...
				try {
					doCommand(getCommandCode(cmd[0]), Integer.parseInt(cmd[1]),
				...
				try {
					...
					if (cmdCode == UVMRobot.COMMAND_SENSOR) {
						...
						doCommand(UVMRobot.COMMAND_START,
	...
	/**
	 * The ActionListener for the RobotGUI textFieldchanges, Timer action,
	 * comboboxchanges, etc
	 * 
	 * 
	 */
	private class RobotGUIActionListener implements ActionListener {
		...
		/**
		 * Handles the timer, buttons, dipscwitchchanges and comboboxchange
		 * events for the RobotGUI
		 * 
		 * @param arg0
		 *            the actionEvent.
		 */
		public void actionPerformed(ActionEvent arg0) {
			...
			if (actionCmd.equals("Reset Hard")
					|| actionCmd.equals("Reset Soft")) {
				// Reset the robot
				...
				robot.reset();
				if (robot instanceof UVMRobot) {
					((UVMRobot) robot).resetRealAgent(arg0.getActionCommand()
							.equals("Reset Hard"));
				}
				return;
			}
			...
			if (actionCmd.equals("comboBoxChanged")) {
				// if first item is selected to nothing since it's just a
				// description like "select a agent"
				if (((JComboBox) arg0.getSource()).getSelectedIndex() == 0) {
					Debug.printInfo("No agent selected");
					return;
				}
				// Agent selected
				if (robot instanceof UVMRobot) { // TODO: document this. Why is
													// this done HERE?
					...
					try {
						...
						if (arg0.getSource().equals(agentCombo)) {
							String agentType = agentCombo.getSelectedItem()
									.toString();
							Class agentClass = ClassLoader
						...
						} else { // other combobox selected so it must be a real
									// UVM agent
									// TODO: the use of this code is not clear
									// yet. Not working.
							String agentType = uvmAgentCombo.getSelectedItem()
									.toString();
							Class agentClass = ClassLoader
									.getSystemClassLoader().loadClass(
											"javaBot.agents." + agentType);
							...
							// UVMRobot r = new UVMRobot("UVMRobot",
							// World.WIDTH/2, World.HEIGHT/2, true);
							// new Thread(new RobotGUI((Robot) r)).start();
						}
					} catch (Exception e) {
						// please note: all robots need to have a parameter-free
						// constructor and need to check regularly
						// if robot.Agent==null)"
						Debug.printError("Error loading agent class)"
								+ e.toString());
					}
		...
		private void displayNewBehaviorValue() {
			...
			txtOutput.setText("   " + behaviorStatus[0] + " + "
					+ behaviorStatus[1] + " + " + behaviorStatus[2] + " + "
					+ behaviorStatus[3] + " = ");
	...
	private void adaptXMLFile() {
		...
		try {
			...
			Pattern paternFile = Pattern
					.compile("<UVMRUNNABLE-FILENAME>+</UVMRUNNABLE-FILENAME>");
			Matcher match = paternFile.matcher(file);

			String dir = System.getProperty("user.dir");

			match.replaceAll("<UVMRUNNABLE-FILENAME>" + dir + "\\"
					+ agentClassName + ".class" + "</UVMRUNNABLE-FILENAME>");
			paternFile = Pattern
					.compile("<UVMRUNNABLE-CLASSFILE>+</UVMRUNNABLE-CLASSFILE>");
			match = paternFile.matcher(file);
			match.replaceAll("<UVMRUNNABLE-CLASSFILE>javaBot.agents."
					+ agentClassName + "</UVMRUNNABLE-CLASSFILE>");
	...
	/**
	 * Create the agent dropdown comboboxes and fill them by reading the
	 * javabot.agents package files
	 * 
	 * @return JPanel the panel with three comoboxes to select Sim, UVM or JPB2
	 *         agents from
	 */
	private JPanel createAgentSelect() {
		...
		agentCombo = fillAgentCombo(new JComboBox(), "agents",
				"javaBot.agents.JobotAgent");
		...
		uvmAgentCombo = fillAgentCombo(new JComboBox(), "agents",
				"com.muvium.UVMRunnable");
		 ...
		 * 
		 * uvmJPB2Combo = new JComboBox(); uvmJPB2Combo.setBounds(5, 70, 160,
		 * 20); uvmJPB2Combo.setBackground(Color.WHITE); // the first item on
		 * pos 0 is just a description
		 ...
		 * cp.add(uvmJPB2Combo);
		 * 
		 * uvmRCJrCombo = new JComboBox(); uvmRCJrCombo.setBounds(5, 100, 160,
		 * 20); uvmRCJrCombo.setBackground(Color.WHITE); // the first item on
		 * pos 0 is just a description
	...
	/**
	 * Fill one of the RobotGUI comboboxes with the classnames found in the
	 * agents package to that an agent can be chosen
	 * 
	 * @param aComboBox
	 *            the new combobox to be filled with items
	 * @param aPath
	 *            path to the agent class files folder
	 * @param aClass
	 *            a baseclass like jobotAgent where the agents must be children
	 *            of
	 * @return JCombobox - the new filled combobox
	 */
	private JComboBox fillAgentCombo(JComboBox aComboBox, String aPath,
			String aClass) {
		java.io.File[] agents;

		try {
			// Get file listing for the javaBot.agents directory
			agents = new java.io.File(Simulator.getRelativePath("./" + aPath
					+ "/")).listFiles();
		} catch (Exception e) {
			Debug.printError("Error in " + aPath
					+ " dropdown, unable to load directory info: "
					+ e.toString());
			return aComboBox;
		}
		...
		for (int i = 0; i < agents.length; i++) {
			if (agents[i].isFile()) {
				...
				try {
					...
					if (cl.loadClass("javaBot." + aPath + "." + agentName)
							.getSuperclass() != null) {
						superClass = cl
								.loadClass("javaBot." + aPath + "." + agentName)
								.getSuperclass().getName();
					}
				} catch (Exception e) {
					Debug.printError("Error in " + aPath
							+ " dropdown, unable to load class: " + agentName
							+ " " + e.toString());
					return aComboBox;
				}

				// If the superclass is an Agent or a UVMRunnable, show it in
				// the dropdown
	...
	private void createDipSwitches(JPanel mainPanel) {
		...
		cp.setBorder(new TitledBorder(null, "  Dip Switches",
		...
		for (int i = 0; i < dipswitches.length; i++) {
			...
			labelswiches[i].setBorder(BorderFactory.createLineBorder(Color.red,
					0));
	...
	private JPanel createLeds() {
		...
		// cp.setBackground(Color.LIGHT_GRAY);
		// cp.setBorder(BorderFactory.createEtchedBorder());
		cp.setBorder(new TitledBorder(null, "  LED Display",
				TitledBorder.LEADING, TitledBorder.TOP, new Font("Dialog",
						Font.PLAIN, 10)));
		cp.setLayout(null);
	...
	private JPanel createLCD(JPanel mainPanel) {
		if (robot.getLCD() != null) {
			...
			cp.setBackground(Color.LIGHT_GRAY);
			cp.setBorder(new TitledBorder(null, "  LCD Display",
					TitledBorder.LEADING, TitledBorder.TOP, new Font("Dialog",
	...
	private void createSensors(JPanel mainPanel) {
		...
		// cp.setBackground(Color.LIGHT_GRAY);
		// cp.setBorder(BorderFactory.createEtchedBorder());
		cp.setBorder(new TitledBorder(null, "  Sensors", TitledBorder.LEADING,
				TitledBorder.TOP, new Font("Dialog", Font.PLAIN, 10)));
		cp.setLayout(null);
		...
		for (int i = 0; i < robot.getSensors().length; i++) {
			...
			JLabel sensor = new JLabel("" + (i) + ". "
					+ robot.getSensors()[i].getName());
	...
	private void createActors(JPanel mainPanel) {
		...
		// cp.setBackground(Color.LIGHT_GRAY);
		// cp.setBorder(BorderFactory.createEtchedBorder());
		cp.setBorder(new TitledBorder(null, "  Actors", TitledBorder.LEADING,
				TitledBorder.TOP, new Font("Dialog", Font.PLAIN, 10)));
		cp.setLayout(null);
		...
		for (int o = 0; o < robot.getActors().length; o++) {
			...
			JLabel actor = new JLabel("" + (o) + ". "
					+ robot.getActors()[o].getName());
			// actor.setBounds(5, (25*o)+(25*i)+(19 * (o + i)) + 5, 160, 20);
			actor.setBounds(5, actorPlacing + 8, 160, 20);
	...
	private void createButtons(JPanel mainPanel) {
		...
		int startPosY = 280 + (robot.getSensors().length * 50)
				+ (robot.getActors().length * 50) + 10;
		int endPosY = 100;

		cp.setBounds(5, startPosY, 170, endPosY);
		// cp.setBackground(Color.LIGHT_GRAY);
		// cp.setBorder(BorderFactory.createEtchedBorder());
		cp.setBorder(new TitledBorder(null, "  Buttons", TitledBorder.LEADING,
				TitledBorder.TOP, new Font("Dialog", Font.PLAIN, 10)));
		cp.setLayout(null);
		...
		for (int i = 0; i < robot.getSensors().length; i++) {
			if (robot.getSensors() != null) {
				...
				if (robot.getSensors()[i].getValue() >= 0) {
					if (robot.getSensors()[i].getName().equals("CompassSensor")) {
						CompassSensor cs = (CompassSensor) robot.getSensors()[i];
						val = cs.getValue(0.0);
					}
					else {
					val = (int) robot.getSensors()[i].getValue();
					}
					// if( val > sensorStatus[i].getMaximum() )
					// sensorStatus[i].setMaximum( val );
	...
	/**
	 * Convert a String to the corresponding UVMRobot commandcode
	 * <p>
	...
	/**
	 * The ActionListener that handles the commands typed in the textfield input
	 * field. <br>
	 * Internal class of RobotGUI
	 * 
	 */
