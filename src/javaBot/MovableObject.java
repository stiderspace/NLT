...
public abstract class MovableObject extends PhysicalObject {
	 ...
	 * collision. Elasticity factor 1 means no energy at all is absorbed. (This
	 * currently only works with collisions with the surrounding walls)
	 */
	 ...
	 */
	public MovableObject(String name, double friction, double locationX,
			double locationY, double diameter, double mass) {
		super(name);
	 ...
	 * 
	 * @param elapsed
	 *            Time since last rendered frame in milliseconds
	 */
	public void update(double elapsed) {
		 ...
		 * sampling
		 */
		velocityX = (Math.abs(velocityX) > Math.abs(dvx)) ? (velocityX + dvx)
				: 0.0;
		velocityY = (Math.abs(velocityY) > Math.abs(dvy)) ? (velocityY + dvy)
				: 0.0;
		...
		/*
		 * When the object is about to pass the surrounding walls put it back
		 * inside
		 * 
		 * to get the new location the diameter is divided bij slightly more
		 * than 2.0 to trigger a collision with the surrounding wall
		 */
		if (newX < 0) {
	...
	public double[] giveSensoryInformationTo(Robot r) {
		...
		for (int i = 0; i < r.getSensors().length; i++) {
			if (r.getSensors()[i] instanceof FieldSensor) {
				...
				Line sensorLine = ds.sensorLine(r.getLocation().getX(), r
			...
			if (r.getSensors()[i] instanceof DistanceSensor) {
				...
				if (dist < (diameter / 2)) {
					d = ds.convertDistanceToValue(new Line(r.location, location)
							.getLength() - (diameter / 2) - (r.diameter / 2));
				}
