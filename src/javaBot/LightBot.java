...
public class LightBot extends Jobot {
	...
	public LightBot(String name) {
		...
		try {
			...
			getSensors()[1] = addSensor("IRSensor", 0, 150); // upper left
																// (northeast)
			getSensors()[2] = addSensor("IRSensor", 0, 30); // upper right
															// (northwest)
			getSensors()[3] = addSensor("ReflectionSensor", BASE_RADIUS, 0, -90); // also
																					// pointing
																					// south
