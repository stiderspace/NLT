...
public class Ball extends MovableObject implements IDragable {
	 ...
	 */
	public Ball() {
		super("ball", 0.5, Math.random() * World.WIDTH, Math.random()
				* World.HEIGHT, 0.1, 1.);
	...
	/**
	 * Checks if any of the objects in the field has an IR sensor and if so, it
	 * gives an approximate value to the sensor based on the distance between
	 * the sensor and the ball.
	 * 
	 * @param r
	...
	public double[] giveSensoryInformationTo(Robot r) {
		...
		for (int i = 0; i < r.getSensors().length; i++) {
			if (r.getSensors()[i] instanceof IRSensor) {
				...
				retval = irs.checkFOV(r.getLocation(), r.rotation,
						this.getLocation());
			...
			if (r.getSensors()[i] instanceof AVRSensor) {
				...
				// Check if sensor has this object within it's field of vision
				retval = avrs.checkFOV(r.getLocation(), r.rotation,
						this.getLocation());
