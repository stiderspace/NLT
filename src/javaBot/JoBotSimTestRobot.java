...
public class JoBotSimTestRobot extends NanoRobot {
	...
	private static final String CONFIG_XML = Simulator
			.getRelativePath("./SimPro/Test/joBotNano.xml");
	 ...
	 */
	public JoBotSimTestRobot(String name, double positionX, double positionY) {
		/*
		...
		try {
			...
			// Create the sensors
			setSensors(new Sensor[13]);

			// addSensor parameters: sensorClassName, radius, position [,
			// direction]
			getSensors()[0] = addSensor("ContactSensor", BASE_RADIUS, 120, 300); // Left
			getSensors()[1] = addSensor("IRSensor", BASE_RADIUS, 270, 270);      // Middle
			getSensors()[2] = addSensor("IRSensor", WHEEL_RADIUS, 300, 290);     // Left
			getSensors()[3] = addSensor("DistanceShortSensor", BASE_RADIUS, 0, 270);
			getSensors()[4] = addSensor("DummySensor", WHEEL_RADIUS, 240, 250);      // Right
			getSensors()[5] = addSensor("VirtualContactSensor", BASE_RADIUS, 240);        // Left
			getSensors()[6] = addSensor("CompassSensor", 0, 270);
			getSensors()[7] = addSensor("ContactSensor", BASE_RADIUS, 60, 240);  // Right
			getSensors()[8] = addSensor("IRSensor", WHEEL_RADIUS, 240, 250);     // Right
			getSensors()[9] = addSensor("FieldSensor", BASE_RADIUS, 300);        // Right
	...
	public double getADCSample(int channel) {
        ...
        if (channel == 6) {   // Dummy sensor used to read heading
//			val = 0.360; //rotation / 100;   // R=1.5 U=0 L=-1.5 T=3.14
			val = getPositionInDegrees(this.rotation);
//			System.out.println("SimTest Chan=" + channel + ",Val="+ val/1000);
			val = val/1000;
		}
		return val;
	}
