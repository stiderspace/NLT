...
public abstract class JobotAgent extends Agent {
	 ...
	 * 
	 * @param vx
	 *            Speed of the horizontal movement, see vy for movement
	 *            perspective
	 * @param vy
	 *            Speed of vertical movement. The movement is in perspective of
	 *            the robot. So if the robot has an angle of 45 degrees, and
	 *            only an y movement is given, then the robot will move in the
	 *            45degrees direction.
	 * @param omega
	 *            Rotation speed and angle of the robot. Positive value will
	 *            rote to the left, negative to the right.
	 */
	public void vectorDrive(double vx, double vy, double omega) {
		double[] a = { (6 / 6.) * Math.PI, (10 / 6.) * Math.PI,
				(2 / 6.) * Math.PI };
		...
		if ((getRobot().getActors() != null)
				&& (getRobot().getActors().length == 3)) {
	...
	protected void heartbeat() {
		if ((System.currentTimeMillis() - lastBeat) > 500) {
			if (getRobot().getLeds().length > 0) {
				getRobot().getLeds()[0].setValue(1 - getRobot().getLeds()[0]
						.getValue());
			}
		}
