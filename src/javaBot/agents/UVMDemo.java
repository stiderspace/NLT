 ...
 * Assignment Sensor Assignment 0 2 1 C C 1 2 0 C is the location of the JPB
 * viewed from the top Wheel movement is as follows: 100 = turn clockwise
 ...
 * execute until it changes manually. A possible next step would be to include
 * an intelligent supervisor which determines which behavior should be presently
 * executing.
 * 
 * @author Peter van Lith, James Caska Ver 0.5 - 07-09-2004 Modified wheel base
 *         Ver 0.6 - 21-09-2004 Modified messages and compiler interface Ver 0.7
 *         - 24-09-2004 Fixed vector drive Ver 0.8 - 04-10-2004 Included
 *         NewBehavior for students Ver 0.9 - 30-12-2004 Included new wall
 *         follower code Included new Flee behavior
 */
public class UVMDemo extends UVMRunnable implements AgentWebService,
	 ...
	 * by muvium as WebService Methods public int getSensor(int sensor); public
	 * int getState(); public void setState(int dip); public void vector(int vx,
	 * int vy, int omega); public void drive(int vx, int vy, int vz);
	 * 
	 * @param vx
	 ...
	 * <code>sensor</code> sensor which represents what the robot currently sees
	 * from this sensor
	 * 
	 * @param sensor
	 *            TODO PARAM: DOCUMENT ME!
	...
	public void setState(int dipValue) {
		...
		case STATE_CALIBRATE:
			currentBehavior = new CalibrateBehavior(joBot, behaviorServiceTick,
					50);
		...
		case STATE_WALL_HUG:
			currentBehavior = new WallHugBehavior(joBot, behaviorServiceTick,
					50);
...
//		System.out.println("UVMdemo V0.9");
		...
		try {
			...
			behaviorServiceTick = getPeripheralFactory().createPeriodicTimer(
					null, 1000, PeripheralFactory.EVENT_PRIORITY_BACKGROUND);
			previousState = -1;
