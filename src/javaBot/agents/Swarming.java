...
import javaBot.SwarmBot;
import javaBot.ReflectionSensor;
import javaBot.Robot;

/**
 * 
 * The BallLover agent wants to go to "distToBall" distance to an IR emitting
 * source
 */
public class Swarming extends JobotAgent {
	...
	/**
	 * construct a new SwarmBot
	 * 
	 * @param robot
	 *            the robot this agent is working on
	 * 
	 */
	public Swarming(Robot robot) {
		super(robot);
		theRobot = robot;
	}
	...
	public void run() {
		...
		// Debug.printDebug(getRobot().name + " preferred distance: " +
		// distToBall);

		if (getRobot() == null) { // this should never happen!
			return;
		}

		// Check if the ballLover is started for the SwarmBot. This is the only
		// bot
		// that has the correct configuration.
		if (getRobot() instanceof SwarmBot) {
			try {
				while (isRunning()) {
					heartbeat();
					startLoving();
				...
				Debug.printError("Error in SwarmBot: " + e.toString());
				e.printStackTrace();
			}
		} else {
			Debug.printInfo("This robot does not support the BallLover. Only the SwarmBot has the correct Sensor configuration.");
		}
	}
	...
	private void findBall() throws Exception {
		...
		if (getRobot().getSensors()[1].getValue() > 10) { // left
			turningSide = 1;
		}

		if (getRobot().getSensors()[2].getValue() > 10) { // right
			turningSide = -1;
		}
	...
	/**
	 * Go towards the ball. sensor 0 and 3 are the south-sensors and point
	 * towards the ball (if it's still there) todo: more documentation on this
	 * method
	 * 
	 * @throws Exception
	 */
	private void driveTowardsBall() throws Exception {
		...
		while (isRunning()) {
			// If we are to far from the ball (detected with the IRSensor) the
			// reflection sensor can't detect the ball
			// the loop will be ended.
			if (getRobot().getSensors()[0].getValue() < 200) {
				// Debug.printDebug("Bot " + getRobot().name +
				// " IRsensor[0] < 200 Value=" +
				// getRobot().getSensors()[0].getValue());
				break;
			}
			if (theRobot instanceof SwarmBot) {
				SwarmBot aBot = (SwarmBot) theRobot;
			    aBot.setColor(Color.GREEN);
			    System.out.println("Set to green: " + aBot.name);
			}
			// The ball is in reach of the reflectionsensor so the distance can
			// be calculated
			...
			// If the distance is zero this means that the ball is detected by
			// the IRSensor but not in front of the Reflectionsensor
			// Check if the preffered distance is reached
			if ((dist < distToBall) && (dist != 0)) {
				// Debug.printDebug("Bot " + getRobot().name +
				// " ReflSensor[3] distance=" + dist + " value=" + svalue);
				break;
			}
			...
			if (theRobot instanceof SwarmBot) {
				SwarmBot aBot = (SwarmBot) theRobot;
			    aBot.setColor(Color.BLUE);
			    System.out.println("Set to blue: " + aBot.name);
			}	
		}
